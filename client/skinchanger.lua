...
function LoadDefaultModel(malePed, cb)
    ...
    CreateThread(function()
        ...
        TriggerEvent('ug-skinChanger:ModelLoaded')
    end)
end
...
AddEventHandler('ug-skinChanger:LoadDefaultModel', function(loadMale, cb)
    LoadDefaultModel(loadMale, cb)
end)

AddEventHandler('ug-skinChanger:GetData', function(cb)
...
AddEventHandler('ug-skinChanger:Change', function(key, val)
    Character[key] = val

    if key == 'sex' then
        TriggerEvent('ug-skinChanger:LoadSkin', Character)
    else
        ApplySkin(Character)
    end
end)

AddEventHandler('ug-skinChanger:GetSkin', function(cb)
    cb(Character)
end)
...
RegisterNetEvent('ug-skinChanger:LoadSkin')
AddEventHandler('ug-skinChanger:LoadSkin', function(skin, cb)
    if skin['sex'] ~= LastSex then
        LoadSkin = skin

        if skin['sex'] == 0 then
            TriggerEvent('ug-skinChanger:LoadDefaultModel', true, cb)
        else
            TriggerEvent('ug-skinChanger:LoadDefaultModel', false, cb)
        end
    else
        ApplySkin(skin)
...
RegisterNetEvent('ug-skinChanger:LoadClothes')
AddEventHandler('ug-skinChanger:LoadClothes', function(playerSkin, clothesSkin)
    if playerSkin['sex'] ~= LastSex then
        ...
        if playerSkin['sex'] == 0 then
            TriggerEvent('ug-skinChanger:LoadDefaultModel', true)
        else
            TriggerEvent('ug-skinChanger:LoadDefaultModel', false)
        end
    else
